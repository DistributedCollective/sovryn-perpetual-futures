...
export function calculateMaintenanceMarginRate(fInitialMarginRateAlpha,
    fMaintenanceMarginRateAlpha, fInitialMarginRateCap, fMarginRateBeta, pos) {
    let cap = fInitialMarginRateCap + fMaintenanceMarginRateAlpha - fInitialMarginRateAlpha
    return Math.min(fMaintenanceMarginRateAlpha + fMarginRateBeta * Math.abs(pos), cap);
}

export function calculateInitialMarginRate(fInitialMarginRateAlpha,
    fMaintenanceMarginRateAlpha, fInitialMarginRateCap, fMarginRateBeta, pos): number {
    return Math.min(fInitialMarginRateAlpha+fMarginRateBeta*Math.abs(pos), fInitialMarginRateCap);
}
 ...
 */
export function getDepositAmountForLvgPosition(pos : number, leverage : number, price : number, S2 : number, S3 : number, S2Mark : number, totalFeeRate : number) {
 ...
 * @param {number} pos0 - target position
 * @param {number} b0 - target position
 * @param {number} tradeAmnt - target position
 ...
 */
 export function getDepositAmountForLvgTrade(pos0 : number, b0 : number, tradeAmnt : number, targetLvg : number, price : number, 
    S3 : number, S2Mark : number) 
{
 ...
 * @returns {number} Amount to be deposited to have the given leverage when trading into position pos
 */
export function calculateLiquidationPriceCollateralBase(LockedInValueQC, position, cash_cc, maintenance_margin_ratio) : number {
    // correct only if markprice = spot price
    // m_r  <= (Sm * Pi - L + cash * S3) / (Sm * |Pi|)
    // -> Sm * (Pi + cash - m_r|Pi|) => L
    return LockedInValueQC / (position - maintenance_margin_ratio * Math.abs(position) + cash_cc);
}
 ...
 * @returns {number} Amount to be deposited to have the given leverage when trading into position pos
 */
export function calculateLiquidationPriceCollateralQuanto(LockedInValueQC, position, cash_cc, maintenance_margin_ratio, S3, Sm) : number {
    ...
    let denominator = maintenance_margin_ratio * Math.abs(position) - position - cash_cc * S3 * Math.sign(position) / Sm;
 ...
 * @returns {number} Amount to be deposited to have the given leverage when trading into position pos
 */
 export function calculateLiquidationPriceCollateralQuote(LockedInValueQC, position, cash_cc, maintenance_margin_ratio) : number {
    // m_r  = (Sm * Pi - L + cash ) / (Sm * |Pi|)
    // -> Sm * (m_r |Pi| - Pi) = - L + cash
    let numerator = -LockedInValueQC + cash_cc;
    let denominator = maintenance_margin_ratio * Math.abs(position) - position;
    return numerator / denominator;
}
 ...
 */
export function isTraderMarginSafe(tau : number, position : number, markPrice : number, lockedInValueQC : number, S2 : number, S3 : number, m : number) : boolean {
 ...
 * @returns {number} Amount to be liquidated (in base currency)
 */
export function calculateLiquidationAmount(marginBalanceCC : number, mntncMarginRate: number, targetMarginRate : number, traderPositionBC : number, liquidationFee : number, tradingFee : number, lotSize : number, S3 : number, S2 : number, Sm : number) {
    // console.assert(mntncMarginRate < targetMarginRate);
    if ( marginBalanceCC * S3 > mntncMarginRate * Math.abs(traderPositionBC) * S2 ) {
        // margin safe
        return 0;
    }
    ...
    let nom = (Math.abs(traderPositionBC) * targetMarginRate * Sm  - marginBalanceCC * S3)
    let deltapos = nom / ( Math.sign(traderPositionBC) * (targetMarginRate * Sm - f * S2) );
    // now round to lot
 ...
 */
export function getMarginBalanceCC(pos : number, LockedInValueQC : number, S2 : number, S3 : number, markPremium : number, cashCC : number, collateralCurrencyIndex : number) : number {
 ...
 * @returns {number} Amount to be liquidated (in base currency)
 */
 export function calculateMarginBalance(position : number, markPrice : number, lockedInValueQC : number, S3 : number, m : number) : number {
 ...
 */
export function isMarginSafe(pos : number, LockedInValueQC : number, cashCC: number, S2: number, S3: number, markPremium: number, collateralCurrencyIndex: number, marginrate: number) : boolean {
    let b2c = getBase2CollateralFX(S2, S3, markPremium, collateralCurrencyIndex, false);
    let M = getMarginBalanceCC(pos, LockedInValueQC, S2, S3, markPremium, cashCC, collateralCurrencyIndex)
    return M > Math.abs(pos) * marginrate * b2c;
}

export function getQuote2CollateralFX(indexS2 : number, indexS3 : number, collateralCurrencyIndex : number) : number {
 ...
 */ 
 export function getBase2CollateralFX(indexS2 : number, indexS3 : number, markPremium : number, collateral_currency_index : number, atMarkPrice : boolean) : number {
...
export function probDefNoQuanto(K2: number,
    L1: number, s2: number, sig2: number, r: number,
...
export function probDefQuanto(K2: number, L1: number, S2: number, S3: number,
    sig2: number, sig3: number, rho: number, r: number,
...
function getVarianceZWithC(r: number, sig2: number, sig3: number, rho: number, C3: number) {
    // helper function for probdefquanto
    return Math.exp(2 * r) * (
        (Math.exp(sig2 ** 2) - 1) * C3 ** 2 + (Math.exp(sig3 ** 2) - 1) +
...
export function roundToLotBN(value: number, lotSize: number): BigNumber {
    ...
    let rounded = div64x64(fValue, fLotSize).add(BigNumber.from("0x8000000000000000"));
...
export function calcKStar(K2: number, L1: number, S2: number, S3: number , M1: number, M2: number, M3: number, rho : number, sig2 : number, sig3 : number) {
    let kStar = M2 - K2;
    if (M3 !== 0) {
        let h = S3/S2 * (Math.exp(rho*sig2*sig3)-1)/(Math.exp(sig2*sig2)-1) * M3;
        kStar = kStar + h;
    }
    return kStar;
}

export function calcPerpPrice(K2: number, k: number,
    L1: number, S2: number, S3: number, sig2: number, sig3: number, rho: number,
    r: number, M1: number, M2: number, M3: number, minimalSpread: number) {
    ...
    if (M3 === 0) {
        ...
        let h = S3/S2 * (Math.exp(rho*sig2*sig3)-1)/(Math.exp(sig2*sig2)-1) * M3
        kStar = kStar + h;
        res = probDefQuanto(K2 + k, L1 + dL, S2, S3, sig2, sig3, rho, r, M1, M2, M3);
    }
...
export function getTradeAmountFromPrice(K2: number, price: number,
    L1: number, S2: number, S3: number, sig2: number, sig3: number, rho: number,
    r: number, M1: number, M2: number, M3: number, minimalSpread: number, lotSize: number, doLog=false) {
    const numIter = 100;
    const dk = 1E-8;
    const fTol = 1E-7;
    
    function getPrice(x : number) {
        let px = calcPerpPrice(K2, x, L1, S2, S3, sig2, sig3, rho, r, M1, M2, M3, minimalSpread);
        return  px;
    }
    ...
    // if(price < midPrice) {
    //     console.assert(price <= getPrice(-lotSize), "below mid price but above 0-short price");
    // }
    // if(price > midPrice) {
    //     console.assert(price >= getPrice(lotSize), "above mid price but below 0-long price");
    // }
    ...
    if(doLog && (getPrice(kd) - price) * (getPrice(ku) - price) >= 0) {
        console.log(kd, getPrice(kd), ku, getPrice(ku), price, getPrice(-lotSize), midPrice, getPrice(lotSize));
    }
    // console.assert((getPrice(kd) - price) * (getPrice(ku) - price) < 0, "valid interval not found");
...
export function getPricesAndTradesForPercentRage(K2: number, L1: number, S2: number, S3: number, sig2: number, sig3: number, rho: number,
    r: number, M1: number, M2: number, M3: number, minimalSpread: number, lotSize: number, pctRange : number[]) {
    // bid-ask spread and mid prices
    let shortPrice0 = calcPerpPrice(K2, -lotSize, L1, S2, S3, sig2, sig3, rho, r, M1, M2, M3, minimalSpread);
    let longPrice0 = calcPerpPrice(K2, lotSize, L1, S2, S3, sig2, sig3, rho, r, M1, M2, M3, minimalSpread);
    ...
    for (let i = 0; i < pctRange.length; i++) {
        ...
        // if(pctRange[i] > 0) {
        //     console.assert(priceRange[i] > longPrice0, "invalid percent range for minimal spread");
        // } else {
        //     console.assert(priceRange[i] < shortPrice0, "invalid percent range for minimal spread");
        // }
        kRange[i] = getTradeAmountFromPrice(K2, priceRange[i], L1, S2, S3, sig2, sig3, rho, r, M1, M2, M3, minimalSpread, lotSize, false);
...
export function calculateFundingRate(premiumRateEWMA: number, S2: number, clamp: number,
    pos: number, AMMFundCashCC: number, collateralCCY: number) {
    ...
    return Math.max(r, clamp) + Math.min(r, -clamp) + Math.sign(-kStar) * baserate;
}
...
function getTargetCollateralM3(K2, S2, S3, L1, sigma2, sigma3, rho23, r, DDTarget) {
    if (K2 === 0) {
        return -1;
    }
    let kappa = L1 / S2 / K2;
    ...
    let c0 = c * qinv2 - kappa ** 2 * Math.exp(-2 * r) + 2 * kappa * Math.exp(-r) - 1;
    let Mstar1 = (-b0 + Math.sqrt(b0 ** 2 - 4 * a0 * c0)) / (2 * a0);
    let Mstar2 = (-b0 - Math.sqrt(b0 ** 2 - 4 * a0 * c0)) / (2 * a0);
    return Math.max(Mstar1, Mstar2);
}

export function calculateAMMTargetSize(DDTarget, CMinimal, sigma2,
    sigma3, rho23, S2, S3, K2, L1, collateralCCY) {
    let M;
    if (collateralCCY === COLLATERAL_CURRENCY_BASE) {
        ...
        M = getTargetCollateralM3(K2, S2, S3, L1, sigma2, sigma3, rho23, 0, DDTarget);
    }
    return Math.max(CMinimal, M);
}

export function getDFTargetSize(K2pair: number[], k2Trader: number, r2pair: number[], r3pair: number[],
    n: number, S2: number, S3: number, collateralCCY: number) {
    ...
    */
    ...
    if (collateralCCY === COLLATERAL_CURRENCY_QUOTE) {
        ...
        return Math.max(loss_down / Math.exp(r2pair[0]), loss_up / Math.exp(r2pair[1]));
    } else {
        // console.assert(collateralCCY === COLLATERAL_CURRENCY_QUANTO);
...
export function equalForPrecision(x: BigNumber, y: BigNumber, decimals: number, doPrintLog = false) {
...
export function equalForPrecisionFloat(x: number, y: number, decimals: number, doPrintLog = false) {
...
export function fractionToABDK64x64(nominator: number, denominator: number) {
    ...
    if (nominator - Math.floor(nominator) !== 0 || denominator - Math.floor(denominator)) {
 ...
 */ 
export function getMaxLeveragePosition(cashBC, targetPremiumRate, alpha, beta, feeRate, slippageRate = null) {
    ...
    let b = targetPremiumRate + slippageRate + feeRate + alpha * (1 - targetPremiumRate);
