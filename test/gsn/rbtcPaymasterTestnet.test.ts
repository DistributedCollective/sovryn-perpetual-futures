...
const btcusdPrice = BigNumber.from("14198494959534290000");
const ethusdPrice = BigNumber.from("2050999358990000000");
...
function computeBtcethPrice() {
    return btcusdPrice.mul("1000000000000000000").div(ethusdPrice);
}

function computeTokenActualCharge(ethActualCharge) {
    return ethActualCharge.mul("1000000000000000000").div(computeBtcethPrice());
}

describe("RbtcPaymaster", () => {
    let owner, user; // wallets
    let mockBtcusdAggregator, mockEthusdAggregator; // mocks
    let mockToken, rbtcPaymaster, mockRelayHub; // contracts
    let relayRequest;

    before(async () => {
        [owner, user] = await ethers.getSigners();
        mockBtcusdAggregator = await deployMockContract(owner, require("../../abi/AggregatorV3Interface.json"));
        mockEthusdAggregator = await deployMockContract(owner, require("../../abi/AggregatorV3Interface.json"));
        await mockBtcusdAggregator.mock.latestRoundData.returns(0, btcusdPrice.toString(), 0, 0, 0);
        await mockEthusdAggregator.mock.latestRoundData.returns(0, ethusdPrice.toString(), 0, 0, 0);
    });

    beforeEach(async () => {
        const MockRelayHub = await ethers.getContractFactory("MockRelayHub");
        const MockToken = await ethers.getContractFactory("MockRbtc");
        const RbtcPaymaster = await ethers.getContractFactory("RbtcPaymasterTestnet");

        mockToken = await MockToken.deploy(ethers.constants.AddressZero);
        rbtcPaymaster = await RbtcPaymaster.deploy(postGasUsage, mockToken.address, mockBtcusdAggregator.address, mockEthusdAggregator.address);
    ...
    describe("Basic functions", () => {
        ...
        it("should set first price feed address at deployment", async () => {
            expect(await rbtcPaymaster.btcusdFeed()).equal(mockBtcusdAggregator.address);
        });

        it("should set second price feed address at deployment", async () => {
            expect(await rbtcPaymaster.bnbusdFeed()).equal(mockEthusdAggregator.address);
        });
    ...
    describe("Internal functions", () => {
        it("should compute token amount needed", async () => {
            const MockRbtcPaymaster = await ethers.getContractFactory("MockRbtcPaymasterTestnet");
            const mockRbtcPaymaster = await MockRbtcPaymaster.deploy(
                postGasUsage,
                mockToken.address,
                mockBtcusdAggregator.address,
                mockEthusdAggregator.address
            );

            const tokenAmountExpected = maxPossibleGas.mul("1000000000000000000").div(computeBtcethPrice());
    ...
    describe("Relay calls", () => {
        describe("PreRelayeCall", () => {
            ...
            before(async () => {
                ...
                preChargeExpected = BigNumber.from(ethMaxCharge).mul("1000000000000000000").div(computeBtcethPrice());
            });
    ...
    describe("Compute postGasUsage value for production", () => {
        it("should return postGasUsage", async () => {
            ...
            await mockRelayHub.calculatePostGasUsage(tokenPreCharge);
        });
    });
});
